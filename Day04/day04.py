...
def str_numbers_to_list(line, char):
    my_split = line.split(char)
    my_list = [int(val) for val in list(filter(None, my_split))]
...
def parse_boards(lines):
    ...
    for line in lines:
        if line != "":  # a new board
            numbers_from_str = str_numbers_to_list_obj(line, ' ')
...
def apply_num_to_boards(boards, number_to_find):
    for x in range(len(boards)):  # each board in boards
        for y in range(len(boards[x])):  # each row in board
            for number in range(len(boards[x][y])):  # each number in row
                if boards[x][y][number]["val"] == number_to_find:
                    boards[x][y][number]["called"] = True


def get_uncalled(row):
    return list(filter(lambda num: num["called"] is False, row))
