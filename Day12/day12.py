...
def init_tree(my_input):
    ...
    for line in my_input:
        split = line.split('-')
        add_new_node(my_nodes, split[0])  # lhs
        add_new_node(my_nodes, split[1])  # rhs
        my_nodes[split[0]].children.add(split[1])
        my_nodes[split[1]].children.add(split[0])
    return my_nodes
...
def can_visit(my_nodes, node_name, max_small_visits):
    if node_name == "start":
        return False
...
def get_path(my_nodes, max_small_visits, all_paths, curr_name, path_so_far):
    ...
    # print(f"curr node {curr_node.name} visited {curr_node.visited}, all paths len {len(all_paths)}, path_so_far {[
    # node.name for node in path_so_far]}")
    curr_node.visited += 1
    ...
    if curr_node.name == "end":
        ...
        for child in curr_node.children:
            if can_visit(my_nodes, child, max_small_visits):
